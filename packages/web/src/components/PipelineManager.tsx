...
export function PipelineManager() {
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        icon={<GitBranch className="w-4 h-4 text-blue-500 dark:text-cyan-400" />}
        title="流水线模板"
        badge="TEMPLATES"
        actions={
      ...
      {/* Template List */}
      {templates.length === 0 ? (
        <div className="text-center py-16 bg-gray-100 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800">
          <GitBranch className="w-8 h-8 text-gray-400 dark:text-gray-700 mx-auto mb-3" />
          <p className="text-sm text-gray-600 dark:text-gray-500">暂无流水线模板</p>
          <p className="text-xs text-gray-400 dark:text-gray-600 mt-1">点击上方按钮创建第一个流水线模板</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left">
              ...
              <tbody className="bg-white dark:bg-gray-950">
                {templates.map(template => (
                  <tr key={template.id} className="border-b border-gray-100 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors">
                    <td className="px-4 py-3 text-gray-500 font-mono text-xs">#{template.id}</td>
                    <td className="px-4 py-3 min-w-0">
                      <div className="flex items-center gap-2">
                        <GitBranch className="w-4 h-4 text-blue-500 dark:text-blue-400 flex-shrink-0" />
                        <div className="min-w-0">
                          <div className="text-sm text-gray-900 dark:text-gray-200 font-medium truncate">{template.name}</div>
                          {template.description && (
                            <div className="text-xs text-gray-500 dark:text-gray-500 truncate">{template.description}</div>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 flex-wrap">
                        {template.phases.map((phase) => (
                          <span key={phase.phaseKey} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-800 text-gray-600 dark:text-gray-400">
                            {phase.label}
                          </span>
                        ))}
                    ...
                    <td className="px-4 py-3 text-xs text-gray-400 dark:text-gray-600">
          ...
          <div className="flex gap-3">
            {COMPLEXITY_OPTIONS.map(opt => (
              <button
                ...
                className="flex-1 flex items-center gap-2 p-3 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 rounded-lg transition-colors text-left"
              >
